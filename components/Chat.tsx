...
export default function Chat() {
  ...
  return (
    <div className="min-h-screen bg-[#16181A] text-[#EDEAE3] font-[system-ui] flex flex-col">
      <div className="max-w-2xl mx-auto w-full px-4 pt-8 pb-28 flex flex-col flex-1">
        <p className="text-xs uppercase tracking-[0.2em] text-[#6B7280] mb-1">Your coach</p>
            ...
            <div key={i} className={m.role === "user" ? "ml-auto max-w-[85%]" : "max-w-[85%]"}>
              <div
                className={`rounded-lg px-3 py-2 text-sm ${
                  m.role === "user" ? "bg-[#7DD3C0] text-[#16181A] whitespace-pre-wrap" : "bg-[#232628] text-[#EDEAE3]"
                }`}
              >
                {m.role === "user" ? m.content : <MarkdownMessage content={m.content} />}
              </div>
              {m.proposedActions && (
                <div className="mt-2 border border-[#3A3F45] rounded-lg p-3 bg-[#1B1D1F]">
                  <p className="text-[10px] uppercase tracking-wide text-[#9AA5B1] mb-2">Proposed plan changes</p>
                  <div className="space-y-3 mb-3">
                    {m.proposedActions.map((a, j) =>
                      a.op === "regenerate" && a.workouts && a.workouts.length > 0 ? (
                        <div key={j}>
                          {a.planMessage && (
                            <div className="text-xs text-[#EDEAE3] mb-2">
                              <MarkdownMessage content={a.planMessage} />
                            </div>
                          )}
                          <p className="text-[10px] uppercase tracking-wide text-[#9AA5B1] mb-1">
                            {a.workouts.length} planned workouts
                          </p>
                          <PlanPreviewGrid workouts={a.workouts} weekFocuses={a.weekFocuses} />
                        </div>
                      ) : (
                        <p key={j} className="text-xs text-[#EDEAE3]">
                          {describeAction(a)}
                        </p>
                      )
                    )}
                  ...
                  {m.actionsResolution === "applied" ? (
                    <p className="text-xs text-[#7DD3C0]">Applied.</p>
                  ) : m.actionsResolution === "discarded" ? (
                    <p className="text-xs text-[#6B7280]">Discarded.</p>
                  ) : (
                    <div className="flex gap-2">
                      <button
                        onClick={() => applyActions(i, m.proposedActions!)}
                        disabled={applyingIndex === i}
                        className="flex items-center gap-1 text-xs px-2.5 py-1.5 rounded-md bg-[#7DD3C0] text-[#16181A] font-medium disabled:opacity-50"
                      >
                      ...
                      <button
                        ...
                        disabled={applyingIndex === i}
                        className="flex items-center gap-1 text-xs px-2.5 py-1.5 rounded-md border border-[#3A3F45] text-[#9AA5B1] hover:bg-[#2A2E32] disabled:opacity-50"
                      >
                        <X size={12} /> Discard
                      </button>
                    </div>
        ...
        {error && (
          <p className="text-xs text-[#E8574B] mb-2 border border-[#E8574B]/40 rounded-md px-3 py-2 bg-[#E8574B]/10">
            {error}
          </p>
        )}
        ...
        <form onSubmit={sendMessage} className="flex gap-2">
          <input
            ...
            className="flex-1 bg-[#1B1D1F] border border-[#2E3236] rounded-md px-3 py-2 text-sm outline-none focus:border-[#7DD3C0]"
          />
          <button
            type="submit"
            disabled={sending || !input.trim()}
            className="px-4 py-2 rounded-md bg-[#7DD3C0] text-[#16181A] text-sm font-medium disabled:opacity-50"
          >
            Send
          </button>
        </form>
